...
const url_info = window.location.href;
const url_info_doc = url_info.slice(38,51)
const url_info_pat = url_info.slice(51,64)
...
class Video extends Component {
	...
	copyUrl = () => {
		...
		if (!navigator.clipboard) {
			...
			try {
				...
				message.success("Meeting URL copied to clipboard!")
			} catch (err) {
		...
		navigator.clipboard.writeText(text).then(function () {
			message.success("MeetingURL copied to clipboard!")
		}, () => {
			message.error("Failed to copy")
		})
	}
	...
	copyTextDoc = () => {
		if (!navigator.clipboard) {
			let textArea = document.createElement("textarea")
			textArea.value = url_info_doc
			document.body.appendChild(textArea)
			textArea.focus()
			textArea.select()
			try {
				document.execCommand('copy')
				message.success("DoctorID copied to clipboard!")
			} catch (err) {
				message.error("Failed to copy")
			}
			document.body.removeChild(textArea)
		...
		navigator.clipboard.writeText(url_info_doc).then(function () {
			message.success("DoctorID copied to clipboard!")
		}, () => {
			message.error("Failed to copy")
		})
	...
	copyTextPat = () => {
		if (!navigator.clipboard) {
			let textArea = document.createElement("textarea")
			textArea.value = url_info_pat
			document.body.appendChild(textArea)
			textArea.focus()
			textArea.select()
			try {
				document.execCommand('copy')
				message.success("PatientID copied to clipboard!")
			} catch (err) {
				message.error("Failed to copy")
			}
			document.body.removeChild(textArea)
		...
		navigator.clipboard.writeText(url_info_pat).then(function () {
			message.success("PatientID copied to clipboard!")
		}, () => {
			message.error("Failed to copy")
		})
	...
	render() {
		...
		return (
			<div>
				<img src={logo} style={{width:"200px", height:"100px", justifyContent: "left", textAlign: "left"}}></img>

				{this.state.askForUsername === true ?
					<div>
						<div style={{background: "white", width: "30%", height: "auto", padding: "20px", minWidth: "400px",
								textAlign: "center", margin: "auto", marginTop: "10px", justifyContent: "center"}}>
							<p style={{ margin: 0, fontWeight: "bold", paddingRight: "50px" }}>Enter your username</p>
						...
						<div style={{ justifyContent: "center", textAlign: "center", paddingTop: "40px" }}>
							<video id="my-video" ref={this.localVideoref} autoPlay muted style={{
								borderStyle: "solid",borderColor: "#bdbdbd",objectFit: "fill",width: "35%",height: "10%"}}></video>
						</div>
					</div>
					:
					<div>
						<div className="btn-down" style={{ backgroundColor: "whitesmoke", color: "whitesmoke", textAlign: "center" }}>
							...
							<Tooltip title="Prescription">
								<IconButton style={{ color: "#424242" }} onClick={() => window.open('https://docit-prescription.netlify.app/')}>
									<LibraryBooksIcon />
								</IconButton>
							</Tooltip>

							<Tooltip title="Whiteboard">
								<IconButton style={{ color: "#424242" }} onClick={() => window.open('https://docit-whiteboard.netlify.app/')}>
									<WebAssetIcon />
								</IconButton>
							</Tooltip>
						...
						<div className="container">
							<div style={{ paddingTop: "20px" }}>
								...
								<Button style={{backgroundColor: "#3f51b5",color: "whitesmoke",marginLeft: "20px",
									marginTop: "10px",width: "50px",fontSize: "10px"
								}} onClick={this.copyUrl}>Copy</Button>
								</Tooltip>
								
								<Tooltip title="Patient ID">
									<Input style={{marginLeft:"15px"}} value={url_info_pat} placeholder="Patient ID" ></Input>
								</Tooltip>
								<Tooltip title="Copy PatientID">
								<Button style={{backgroundColor: "#3f51b5",color: "whitesmoke",marginLeft: "20px",
									marginTop: "10px",width: "50px",fontSize: "10px"
								}} onClick={this.copyTextPat}>Copy</Button>
								</Tooltip>

								<Tooltip title="Doctor ID">
									<Input style={{marginLeft:"15px"}} value={url_info_doc} placeholder="Doctor ID" ></Input>
								</Tooltip>
								<Tooltip title="Copy DoctorID">
								<Button style={{backgroundColor: "#3f51b5",color: "whitesmoke",marginLeft: "20px",
									marginTop: "10px",width: "50px",fontSize: "10px"
								}} onClick={this.copyTextDoc}>Copy</Button>
								</Tooltip>
							</div>
